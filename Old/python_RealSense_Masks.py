...
# Define camera extrinsic parameters (transformation matrix from camera frame to camera frame)
            ...
            if result.masks:
                ...
                if boxes is not None:
                    ...
                    for i, (mask, obj_id, obj_class) in enumerate(zip(masks, ids, classes)):
                        ...
                        # Convert depth and pixel coordinates to 3D camera coordinates
                        x = (center_x - cx) * average_depth / fx
                        y = -(center_y - cy) * average_depth / fy
                        z = average_depth

                        # Transform to camera coordinates
                        camera_coordinates = np.dot(camera_transform, np.array([x, y, z, 1]))
                        camera_x, camera_y, camera_z = camera_coordinates[:3]

                        # Append to lists
                        x_camera.append(camera_x)
                        y_camera.append(camera_y)
                        z_camera.append(camera_z)

                        # Append object ID and name to the list
                        object_ids.append(f"ID {obj_id}")

                        if draw:
                            # Annotate the frame with the object's camera position at the centroid of the mask
                            label = f"ID {obj_id} ({obj_name}):\n({camera_x:.2f}, {camera_y:.2f}, {camera_z:.2f})"
                            ...
                            for j, line in enumerate(lines):
                                ...
                                # Use cv2.putText to add camera position information at the center of the mask
                                cv2.putText(annotated_frame, line, (text_x, text_y), 
                                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2, cv2.LINE_AA)

        # Update the 3D plot
        if plot:
            update_plot(x_camera, y_camera, z_camera, object_ids)

        # Display the annotated frame with masks
        if draw:
            cv2.imshow("YOLOv9 Detection with camera Position and Masks", annotated_frame)
