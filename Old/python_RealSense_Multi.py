...
show = True
draw = False  # Set to True if you want to draw bounding boxes or masks on the frames
plot = False  # Set to True if you want to plot the 3D positions of detected objects
...
pipelines, aligns, profiles = initialize_cameras(serials=serials, resolution=[640, 360], fps=15)
...
detection_list = [None] * len(pipelines)
...
def process_camera(pipeline, align, profile, model, idx, camera_transform):
    ...
    while True:
        ...
        detection_list[idx] = (x_robot, z_robot, y_robot, object_ids, object_names, idx+1)
...
try:
    while True:
        ...
        if show and all(frame is not None for frame in annotated_frames):
            try:
                # Reorder frames: Move frames 4 and 5 to the beginning
                reordered_frames = annotated_frames[3:5] + annotated_frames[0:3]
            except:
                reordered_frames = annotated_frames

            # Horizontally stack the reordered frames
        ...
        if plot and all(coords is not None for coords in detection_list):
            for (scatter, detection) in zip(scatters, detection_list):
                scatter._offsets3d = detection[:3]  # Update scatter plot data with robot coordinates
                plt.draw()  # Redraw the plot
                plt.pause(0.001)  # Pause to update the plot

        # print(detection_list) # X, Y, Z, IDs, Names, Camera
